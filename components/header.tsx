      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="flex h-20 items-center justify-between">
          ...
          <nav className="hidden lg:flex items-center gap-8">
                  ...
                  <div className="bg-card border border-border rounded-xl shadow-2xl p-6 min-w-[750px]">
                    <div className="grid grid-cols-2 gap-4">
                      {servicesData.map((service) => (
                        <Link key={service.slug} href={`/leistungen/${service.slug}`} className="flex items-start gap-4 p-3 rounded-lg hover:bg-secondary transition-colors group">
            ...
            {[["Über Uns", "/#ueber-uns"], ["Partner", "/#partner"], ["Zertifikate", "/#zertifikate"], ["Kontakt", "/#kontakt"]].map(([label, href]) => (
              <Link key={label} href={href} className="text-sm font-medium text-muted-foreground hover:text-foreground transition-colors">{label}</Link>
            ))}
          </nav>
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden py-4 border-t border-border">
            <nav className="flex flex-col gap-2">
                  ...
                  <div className="pl-4 pb-2 flex flex-col gap-2">
                    {servicesData.map((service) => (
                      <Link key={service.slug} href={`/leistungen/${service.slug}`} className="flex items-center gap-3 py-2 text-sm text-muted-foreground hover:text-foreground transition-colors" onClick={() => setMobileMenuOpen(false)}>
                        <service.icon className="h-4 w-4 text-primary" /> {service.title}
                      </Link>
                    ))}
                  </div>
                )}
              ...
              {[["Über Uns", "/#ueber-uns"], ["Partner", "/#partner"], ["Zertifikate", "/#zertifikate"], ["Kontakt", "/#kontakt"]].map(([label, href]) => (
                <Link key={label} href={href} className="py-2 text-sm font-medium text-muted-foreground hover:text-foreground transition-colors" onClick={() => setMobileMenuOpen(false)}>{label}</Link>
              ))}
